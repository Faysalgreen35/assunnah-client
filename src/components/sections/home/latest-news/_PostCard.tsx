      ...
      >
        ...
        <div className="flex flex-col justify-between flex-1 p-4">
          <div>
            ...
            <h3 className="mt-2 text-[13px] font-bold leading-5 text-heading group-hover:text-primary transition-colors line-clamp-2">{post.title}</h3>
            <p className="mt-2 text-[11px] text-muted line-clamp-2 leading-4">{post.excerpt}</p>
          </div>
          {post.publishDate && (
            <p className="mt-3 text-[10px] text-subtle">{new Date(post.publishDate).toLocaleDateString()}</p>
          )}
        </div>
      </Link>
    );
